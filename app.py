...
st.markdown("""
    ...
    .gym-card-container {
        ...
        padding: 16px !important; margin-bottom: 8px !important; background-color: #F8F9FA !important;
        border-radius: 8px !important; border: 1px solid #E9ECEF !important;
        text-decoration: none !important; color: #1A1A1A !important;
    }
    .gym-name-main { font-weight: 700 !important; font-size: 1.05rem !important; }
    .gym-last-date { color: #666 !important; font-size: 0.85rem !important; }
    </style>
...
with tab2:
        ...
        with st.form("log_form", clear_on_submit=True):
            ...
            if l_gym != "(選択)" and not log_df.empty:
                ...
                if not gym_logs.empty:
                    ...
                    st.info(f"💡 前回訪問: {last_v.strftime('%m/%d')} ({(datetime.now().date() - last_v.date()).days}日前)")
                st.markdown(f"🏆 今月このジムに来た回数: **{len(gym_logs[gym_logs['date'].dt.strftime('%Y-%m') == datetime.now().strftime('%Y-%m')]) + 1}回目**")
    ...
    if not log_df.empty:
        ...
        # 自由な期間指定
        c1, c2 = st.columns(2)
        with c1: start_q = st.date_input("開始日", value=date(datetime.now().year, datetime.now().month, 1))
        with c2: end_q = st.date_input("終了日", value=datetime.now().date())
        ...
        if not disp_df.empty:
            ...
            counts['legend_label'] = counts.apply(lambda x: f"{x['gym_name']} 【{x['count']}】", axis=1)
            
            fig = px.pie(counts, values='count', names='legend_label', hole=0.6, color_discrete_sequence=px.colors.qualitative.Pastel)
            fig.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=300, legend=dict(orientation="h", yanchor="bottom", y=-0.5))
...
with tab3:
    ...
    for gym in sorted_gyms:
        ...
        st.markdown(f"""
            <a href="{url}" target="_blank" class="gym-card-container">
                <span class="gym-name-main">{gym}</span>
                <span class="gym-last-date">Last: {v_date}</span>
            </a>
        """, unsafe_allow_html=True)
