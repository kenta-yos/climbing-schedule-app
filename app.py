...
with tab1:
    ...
    if not schedule_df.empty:
        ...
        for _, row in m_df.sort_values(['is_past', 'start_date']).iterrows():
            ...
            st.markdown(f"""
                <a href="{row['post_url']}" target="_blank" class="custom-list-item {past_cls}">
                    <div class="custom-accent"></div>
                    <span class="custom-date">{d_disp}</span>
                    <span class="custom-gym">{row['gym_name']}</span>
                </a>
            """, unsafe_allow_html=True)
            ...
            for _, row in disp_df.sort_values('date', ascending=False).iterrows():
                st.markdown(f"""
                    <div class="custom-list-item">
                        <div class="custom-accent"></div>
                        <span class="custom-date">{row['date'].strftime('%m/%d')}</span>
                        <span class="custom-gym">{row['gym_name']}</span>
                    </div>
                """, unsafe_allow_html=True)
