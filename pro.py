...
st.markdown("""
    ...
    .gym-row-pro { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid #EEE; text-decoration: none; }
...
# 日付変換
if not schedule_df.empty: schedule_df['start_date'] = pd.to_datetime(schedule_df['start_date'])
if not log_df.empty: log_df['date'] = pd.to_datetime(log_df['date'], errors='coerce')  # 日付形式エラー回避
...
with tab1:
    ...
    for gym in ranked[:10]:
        # HTML生成安全版
        reason_html = " ".join([f'<span class="tag-item {"tag-hot" if "🔥" in r else ""}">{r}</span>' for r in gym["reasons"]])
        gym_html = f'''
        <div class="gym-row-pro">
            <div class="gym-info-main">
                <a href="{gym["url"]}" target="_blank" class="gym-name-link">{gym["name"]} ({gym["area"]})</a>
                <div class="gym-tags">{reason_html}</div>
            </div>
        </div>
        '''
        st.markdown(gym_html, unsafe_allow_html=True)
...
with tab3:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f'<div class="insta-card"><div class="insta-label">Total</div><div class="insta-val">{len(df_res)}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="insta-card"><div class="insta-label">Gyms</div><div class="insta-val">{df_res["gym_name"].nunique() if not df_res.empty else 0}</div></div>', unsafe_allow_html=True)
    if not df_res.empty:
