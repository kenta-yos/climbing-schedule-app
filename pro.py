...
def load_data():
    try:
        ...
        st.warning("Google Sheets 接続待機中...")
...
with tabs[1]:
    ...
    def get_ranked_gyms(t_date):
        ...
        for _, gym in gym_df.iterrows():
            ...
            if not others.empty:
                score += (15 * len(others))
                reasons.append(f"👥 {len(others)}名の予定")
...
# ==========================================
with tabs[3]:
    st.subheader("👥 仲間の予定 (1ヶ月)")
    limit_dt = pd.to_datetime(date.today()) + timedelta(days=30)
    others = log_df[(log_df['user'] != st.session_state.USER) & (log_df['type'] == '予定') & (log_df['date'].dt.date >= date.today()) & (log_df['date'] <= limit_dt)].sort_values('date')
